...
declare module "react-pixi-fiber" {
  ...
  // @ts-ignore TS2694
  type InteractionCompatibility = Exclude<
    keyof typeof PIXI.interaction,
    number | symbol
  >;
  ...
  type KeysThatMayHaveType<T, U> = {
    [K in keyof T]: U extends T[K] ? K : never;
  }[keyof T];

  // Returns keys `K` of `T` where type of `T[K]` is not specifically `any`.
  type KeysThatAreNotAny<T> = {
    [K in keyof T]: any extends T[K] ? never : K;
  }[keyof T];

  // The shape of `T` with `children` property that React understands.
  type PropsWithReactChildren<T> = Omit<T, "children"> & {
  ...
  export type PointProperties<T> = Extract<
    keyof T,
    Extract<KeysThatMayHaveType<T, PixiPoint>, KeysThatAreNotAny<T>>
  >;
  ...
  export type DisplayObjectProps<T> = PropsWithReactChildren<
    Partial<WithPointLike<T>>
  >;
  ...
  export type StageAsCanvasProps =
  ...
  export type StageProps = Omit<
    (StagePropsWithApp | StagePropsWithOptions) &
      StageAsCanvasProps &
      StageAsContainerProps,
    "height" | "width"
  >;
  ...
  export type Stage = React.ForwardRefExoticComponent<
    StageProps & { ref?: React.Ref<StageRef> }
  > &
    StageRef;
  ...
  export type CustomDisplayObjectCreator<T extends PIXI.DisplayObject, P> = (
    props: P
  ) => CustomDisplayObject<T, P>;
  ...
  export type CustomDisplayObjectAttachHandler<T extends PIXI.DisplayObject> = (
    displayObject: T
  ) => void;
  ...
  export type CustomDisplayObjectDetachHandler<T extends PIXI.DisplayObject> = (
    displayObject: T
  ) => void;

  // Inject API adds `_customApplyProps`, `_customDidAttach`, `_customWillDetach` methods.
  export interface CustomDisplayObject<T extends PIXI.DisplayObject, P>
  ...
  export interface CustomPIXIComponentBehaviorDefinition<
