...
if (__DEV__) {
   ...
   */
  CustomPIXIProperty = function CustomPIXIProperty(
    maybeComponentType,
    propertyName,
    validator
  ) {
    ...
    componentTypeList.forEach((componentType) => {
      ...
      invariant(
        !(
          ...
          !Object.keys(possibleStandardNames[componentType]).includes(
      ...
      possibleStandardNames[componentType][lowerCasedPropertyName] =
        propertyName;
      // inject custom property info
      if (!Object.keys(customProperties).includes(componentType)) {
        customProperties[componentType] = {};
      }
      customProperties[componentType][propertyName] = new PropertyInfoRecord(
        propertyName,
        validator
      );
    });
